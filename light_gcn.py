...
class EnhancedRecommendationSystem:
    ...
    def find_similar_users(self, target_user_data: Dict, all_users: List[Dict],
                           user_interactions: Union[List[str], List[Dict]] = None,
                           top_k_users: int = 3) -> Dict:
        if not self.is_trained:
            raise ValueError("Model must be trained first!")

        self.model.eval()
        with torch.no_grad():
            # Get embedding for the target user
    ...
    def save_model(self, model_path: str = "lightgcn_model.pt", mappings_path: str = "mappings.pkl"):
        ...
        mappings_data = {
            ...
            'final_user_emb': self.model.final_user_emb,
            'final_item_emb': self.model.final_item_emb
        }
        with open(mappings_path, 'wb') as f:
            pickle.dump(mappings_data, f)
        logger.info(f"Model state saved to {model_path}")
        logger.info(f"Mappings and embeddings saved to {mappings_path}")
    ...
    def load_model(cls, model_path: str = "lightgcn_model.pt", mappings_path: str = "mappings.pkl"):
        ...
        try:
            ...
            rec_system.model = LightGCNWithUserAndItemInfo(
                num_users=mappings_data['num_users'],
                num_items=mappings_data['num_items'],
                recovery_vocab=len(mappings_data['recovery_stage_map']),
                type_vocab=len(mappings_data['preferred_type_map']),
                resource_type_vocab=len(mappings_data['resource_type_map']),
                embedding_dim=rec_system.embedding_dim,
                feature_dim=rec_system.feature_dim,
                num_layers=rec_system.num_layers
            )
            state_dict = torch.load(model_path, map_location=torch.device("cpu"))
            rec_system.model.load_state_dict(state_dict)
            rec_system.model.final_user_emb = mappings_data['final_user_emb']
            rec_system.model.final_item_emb = mappings_data['final_item_emb']
